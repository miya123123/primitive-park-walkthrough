...
Godot 4.6.2で作った、プリミティブ形状だけの72m四方の遊園地を、来園者モデルとともに三人称視点で歩き回るPC/ブラウザ向け3Dゲームです。外部モデル・テクスチャ・音声は使用せず、JSON設定を読み込んでGDScriptのファクトリが箱、円柱、球、カプセルから園内を生成します。

ブラウザ版デモ: [GitHub Pagesで遊ぶ](https://miya123123.github.io/primitive-park-walkthrough/)
...
Godotエディターからは `scenes/Main.tscn` を開き、プロジェクトを実行してください。
...
- `E`: 乗り場で乗車（自動運行は1周後に降車、ゴーカートは手動退出）
- `R`: ゴーカートを最後に通過したチェックポイントへリセット
- `Esc`: マウスカーソルを解放（左クリックで再取得）

園内には観覧車、ジェットコースター、メリーゴーラウンド、フリーフォールタワー、ゴーカートコースがあります。フリーフォールは上昇後に2.5〜4.5秒のランダムな「間」を置いてから落下します。ゴーカートはWASDで運転する3周タイムアタックで、ラップ・経過時間・ベストタイムをHUDに表示し、ベスト記録を `user://primitive_park_records.cfg` に保存します。設定は `assets/data/park_config.json`、実装は `src/`、設計・QA資料は `design/`・`docs/`・`production/` にあります。

テストは依存関係のないGDScriptランナーで実行できます。
